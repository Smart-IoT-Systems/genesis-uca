...
var smoolkp = function (spec) {
    ...
    that._configure = async function(){
        const rl = readline.createInterface({ input: fs.createReadStream(that.file), output: process.stdout });
        ...
        const rl2 = readline.createInterface({ input: fs.createReadStream(path_checker), output: process.stdout });
        var pos_reader = 0;
